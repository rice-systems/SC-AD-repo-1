                              ...
                              uint4* keysIn,  uint4* valuesIn)
{
    ...
    const unsigned long i = (unsigned long) blockIdx.x * blockDim.x + threadIdx.x;
        ...
        uint* blockOffsets, uint startbit, uint numElements, uint totalBlocks)
{
    ...
    uint2 radix2;
    radix2 = keys[(unsigned long) blockIdx.x * blockDim.x + threadIdx.x];
                            ...
                            uint  totalBlocks)
{
    ...
    unsigned long i = (unsigned long) blockId * blockDim.x + threadIdx.x;
     ...
     const bool fullBlock, const bool storeSum)
{
    ...
    const unsigned long gid = (unsigned long) blockIdx.x * blockDim.x + threadIdx.x;
    const unsigned long tid = threadIdx.x;
    const unsigned long i = gid * 4;
